...
namespace TeePee.Examples.WebApp.Tests.TestScopedTests
{
    public class HttpClientFactoryMultipleNamedUsageControllerTests
    {
        ...
        public async Task ManualInjection_RecommendedPassiveMocking()
        {
            // Given
            m_TeePeeBuilderOne
                .ForRequest("https://first.api/path-one/resource-one", HttpMethod.Get)
            ...
            m_TeePeeBuilderTwo
                .ForRequest("https://second.api/path-two/resource-two", HttpMethod.Get)
                .ThatContainsQueryParam("filter", "those")
                .Responds()
                .WithStatus(HttpStatusCode.OK)
                .WithBody(new
            ...
            var controller = new HttpClientFactoryMultipleNamedUsageController(new[]
                    {
                        ((_EXPECTED_NAMED_HTTP_CLIENT_ONE, m_TeePeeBuilderOne.Manual("https://first.api"))),
                        ((_EXPECTED_NAMED_HTTP_CLIENT_TWO, m_TeePeeBuilderTwo.Manual("https://second.api")))
                    }
                    .ToHttpClientFactory());
        ...
        public async Task ManualInjection_MockAndVerify()
        {
            ...
            var requestTrackerOne = m_TeePeeBuilderOne
                .ForRequest("https://first.api/path-one/resource-one", HttpMethod.Put)
            ...
            var requestTrackerTwo = m_TeePeeBuilderTwo
                .ForRequest("https://second.api/path-two/resource-two", HttpMethod.Put)
                .ThatContainsQueryParam("filter", "other")
                .ThatHasBody(new { Caller = "ThisCaller" })
                .Responds()
                .WithStatus(HttpStatusCode.Created)
